...
elseif strcmp(action, 'set_gui'),   % set figure, axes and gui's
    ...
    FcmFigH = figure('Name', FcmFigTitle, 'NumberTitle', 'off',...
        'DockControls','off','Resize','off','Toolbar','none');
...
%       if max(max(U_prev - FcmU)) < min_eps, break; end,
        ...
        center_prev = FcmCenter;
        U_prev = FcmU;
        % store FCM center for plotting whole trajectory (R2011A)
        for j = 1:cluster_n
            Center{j} = [Center{j};FcmCenter(j,:)];
        end
    end
    % plot trajectory
    for j = 1:cluster_n
        LineData = Center{j};
        line(LineData(:,1),LineData(:,2),'color',get(headH(j),'color'),'LineStyle', 'none', 'Marker', '.', 'markersize', 30, 'tag', 'trajHead');
        line(LineData(:,1),LineData(:,2),'color',get(headH(j),'color'),'linewidth',3, 'tag', 'trajLine');
    end
...
elseif strcmp(action, 'cluster_number'),
    ...
    delete(findobj(FcmFigH, 'tag', 'trajHead'));
    delete(findobj(FcmFigH, 'tag', 'trajLine'));
    delete(findobj(FcmFigH, 'tag', 'selectH'));
    FcmCenter = [];
    set(findobj(FcmFigH, 'tag', 'mf_plot'), 'userdata', []);
    eval([mfilename, '(''label_data0'')']);
...
elseif strcmp(action, 'mf_plot'),
    ...
    if isempty(which_cluster),
	    ...
	    colordef(MfFigH, 'black');
            mesh(XI, YI, ZI);
            xlabel('X','fontsize',10), ylabel('Y','fontsize',10), zlabel('MF','fontsize',10);
